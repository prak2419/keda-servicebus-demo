...
## Integration with ServiceBus queue through pod identity
...
- Helm to add and install the KEDA repo.
- Separate namespace for KEDA to decouple its components from the application namespace.
- A pod identity binding for the auto-scaler CRD
...
3. Create a namespace for KEDA and Install keda Helm chart
    ...
    * Install the helm repo by setting a label for the identity created for the KEDA auto-scaler (`app-autoscaler`).
...
4. Create an identity and identity binding for the KEDA auto-scaler by running the `scaler_binding.yml` to the KEDA namespace after updating the proper values. Update the manifest section highlighted below to match the label applied for the identity from the previous step on the identity binding kind.
    ...
    spec:
      ...
      <b>selector: app-autoscaler</b>

    </pre>

    ```bash

    kubectl apply -f scaler_binding.yml -n <keda_namespace>
...
3. If the scaling has to be handled as a deployment, we need to perform the following steps,
    ...
    * Modify the values of Azure Container Registry and the AAD pod binding label on the `deployment.yml` file.
    * Apply the deployment to the cluster, assuming the user is already authenticated to AKS cluster
    ...
    * Update the `scale_deployment.yml` file with the value of your queue name and service bus namespace. The `maxreplicaCount` value controls the maximum number of pods allowed to run as part of the KEDA auto-scaling.
    * Deploy the `ScaledObject` to the cluster
...
4. If the scaling has to be deployed as jobs, we need to perform the following steps,
    ...
    * Update the `scale_job.yml` with appropriate job related configuration and also change the ACR/queue/namespace/aadpodbinding names.
    * Deploy the `ScaledJob` to the cluster
